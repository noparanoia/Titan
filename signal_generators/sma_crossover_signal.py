...
class SmaCrossoverSignal(BaseSignalGenerator):
    ...
    def __init__(self, market, interval, sma_short, sma_long, strategy):
        ...
        self.fma = simple_moving_average.SimpleMovingAverage(
            self.market, interval, sma_short)
        self.sma = simple_moving_average.SimpleMovingAverage(
            self.market, interval, sma_long)
        self.vol_change = volume_change_monitor.VolumeChangeMonitor(
    ...
    def check_condition(self, new_candle):
        ...
        if (self.sma.value is not None) & (self.fma.value is not None) & (self.vol_change.value is not None):
            ...
            self.strategy.print_message(
                "VOL Change: " + str(self.vol_change.value) + "%")
            ...
            if self.cached_high is not None:
                self.strategy.print_message(
                    "Checking if current price is greater than cached high")
                if not self.fma.value > self.sma.value:  # if we're no longer fma > sma, forget about saved high
                    self.strategy.print_message(
                        "FMA has gone below SMA, forgetting cached high")
                    self.cached_high = None
                    return False
                # open a trade if the latest high is greater than the cached high
                if new_candle[2] > self.cached_high:
                    self.strategy.print_message("Current high of " + str(
                        new_candle[2]) + " has exceeded cached high of " + str(self.cached_high) + ", buy signal generated")
                    self.cached_high = None
                    return True
                else:
                    return False
            ...
            # if fma is not already above sma, and has now crossed, and volume is up 5% from last period, send trade signal
                ...
                self.strategy.print_message(
                    "FMA has crossed SMA, caching current high of " + str(new_candle[2]))
